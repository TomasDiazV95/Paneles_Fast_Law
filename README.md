...
└── Frontend/
    ...
    └── src/
        ...
        │       ├── cenco/    (tabs: estado cartera, contactabilidad, pagos, repros, comparativo, ejecutivos, salidas, estado cyber)
...
- Acceso de red al servidor SQL Server correspondiente, incluyendo, para los reportes de La Araucana y para las tabs Salidas/Estado Cyber del Panel CENCO, al servidor vinculado `PROMETEO\FASTCO` / base `SISTEMA_JFASTCO`, referenciado directamente en las consultas de `Backend/app/routers/panel_araucana.py` y en los stored procedures `SP_Panel_Cenco_Salidas`/`SP_Panel_Cenco_Estado_Cyber` (`Backend/scripts_sql/cenco/`).
...
- **Panel CENCO** (`PanelCenco.jsx`): selector de cartera (H1 / T4) y período; tabs de Estado Cartera, Contactabilidad, Pagos, Reprogramaciones, Comparativo, Ejecutivos, Salidas y Estado Cyber. La tab **Salidas** lista los casos con salida judicial vigente (marca de cierre/suspensión de juicio, detección de duplicados) y permite descargar Excel. La tab **Estado Cyber** muestra 3 tarjetas KPI clicables (Actualizado / No Actualizado / Total) que también filtran una tabla paginada en pantalla (máximo 15 filas por página, paginación server-side) con el cruce entre el stock, la última gestión de autogestión judicial y el resultado de la sábana judicial homologado contra una tabla de homologación, y permite descargar a Excel el detalle filtrado (sin límite de filas).
...
- `GET /panel/cenco/salidas` — listado de casos con salida judicial vigente (marca de cierre/suspensión de juicio, indicador de duplicados), filtrado por `periodo` (ejecuta el stored procedure `SP_Panel_Cenco_Salidas`).
- `GET /panel/cenco/salidas/descarga` — descarga Excel (columnas `NUMERO_JUICIO`, `MARCA`) del mismo listado, filtrado por `periodo`.
- `GET /panel/cenco/estado-cyber` — resumen (`Q_ACTUALIZADO`, `Q_NO_ACTUALIZADO`, `Q_TOTAL`) y detalle paginado del cruce entre stock, última gestión de autogestión judicial y sábana judicial homologada contra una tabla de homologación Cyber, para determinar si cada caso quedó "ACTUALIZADO" o "NO ACTUALIZADO"; filtrado por `periodo`, `producto` (default `5`), `estado` opcional (`ACTUALIZADO`/`NO ACTUALIZADO`, 400 si es otro valor), `pagina` y `tamano_pagina` (acotado a máx. 15 en este endpoint) (ejecuta el stored procedure `SP_Panel_Cenco_Estado_Cyber`).
- `GET /panel/cenco/estado-cyber/descarga` — descarga Excel con el detalle completo (sin límite de 15 filas) del mismo cruce, filtrado por `periodo`, `producto` y `estado`.
...
- Los endpoints `/panel/cenco/salidas` y `/panel/cenco/estado-cyber` (y sus respectivas descargas) ejecutan los stored procedures `SP_Panel_Cenco_Salidas` y `SP_Panel_Cenco_Estado_Cyber` (`Backend/scripts_sql/cenco/`), que consultan además tablas propias de CENCO fuera del prefijo `PANEL_CENCO_*`: `TBL_STOCK_CENCO`, `TBL_AUTOGES_CENCO`, `TBL_SALIDAS_CENCO` y `TBL_HOMOLOGACION_CYBER_JFASTCO`. Esta última no tiene DDL en el repositorio (se asume existente en la base de datos real, con al menos las columnas `ID_HOMOLOGACION`, `DESC_CODIGO_RESULTADO`, `RESPUESTA_JFASTCO` y `ACTIVO`); si se encuentra o genera su DDL real, debería agregarse como script en `Backend/scripts_sql/cenco/`.
- Los endpoints de descarga de La Araucana, y los stored procedures `SP_Panel_Cenco_Salidas`/`SP_Panel_Cenco_Estado_Cyber` de CENCO, consultan además un servidor vinculado (linked server) identificado en el código como `[PROMETEO\FASTCO].[SISTEMA_JFASTCO]`, con tablas como `tbl_resultados_sabana` (además de `TBL_CLASIFICACION_ESTADOS` y `TBL_JUICIO` para La Araucana). Esto implica que, además de la base principal, el entorno donde corra el backend debe tener conectividad y permisos hacia ese servidor vinculado.
